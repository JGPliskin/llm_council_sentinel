...
│                              用户浏览器 (User Browser)                           │
│  ┌─────────────────────┐   ┌─────────────────────┐   ┌─────────────────────┐    │
│  │    输入区域         │   │   Stage 渲染区域     │   │    Detail Panel    │    │
│  │ (WelcomeScreen)     │   │ (StageContentArea)  │   │  (评审/思考详情)    │    │
│  └─────────────────────┘   └─────────────────────┘   └─────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        │ HTTP REST / SSE
...
graph TD
    ...
    logger.py --> config.py
    model_assigner.py --> health.py
    model_assigner.py --> config.py
```
...
| **WelcomeScreen** | `WelcomeScreen.jsx` | 会话启动与 Councilor 选择 |
| **StageContentArea** | `StageContentArea.jsx` | Stage1/Stage3 内容展示与 Thinking 展开 |
| **ConsensusBeacon** | `ConsensusBeacon.jsx` | 共识提示与跳转入口 |
| **TacticalHUD** | `TacticalHUD.jsx` | 底部状态栏、Councilor 状态、共识信号 |
| **DetailPanel** | `DetailPanel.jsx` | 评审/主席思考详情面板 |
...
| 函数 | 说明 |
|---|---|
| `getCouncilors(refresh)` | 获取 Councilor 列表 |
| `getModels(refresh)` | 获取 Councilor 列表（Legacy） |
| `listConversations()` | 获取所有对话 |
...
| `sendMessageStream(id, content, onEvent, councilorIds, enableThinking)` | 发送消息（SSE 流式） |
...
│  │                        docker-compose                                │    │
│  │  ┌────────────────┐  ┌────────────────┐  ┌────────────────────────┐ │    │
│  │  │   Nginx:80     │  │  Backend:8008  │  │  Frontend (Build)      │ │    │
...
**动态模式（无固定分配）**：
- 候选池过滤为 `healthy/unknown`，按 TTFT 速度排序选最快。
