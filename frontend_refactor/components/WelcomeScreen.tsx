...
import { MOCK_AGENTS, CHAIRPERSON, PROTOCOL_PRESETS } from '../mockData';
...
export const WelcomeScreen = ({ onStart, selectedAgents, onToggleAgent }: WelcomeProps) => {
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if(inputValue.trim() && selectedAgents.length > 0) onStart(inputValue);
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center relative overflow-y-auto overflow-x-hidden bg-zinc-950 p-4 min-h-full">
       {/* Background */}
       <div className="absolute inset-0 bg-grid-pattern opacity-10 animate-[pulse-glow_4s_infinite] pointer-events-none"></div>
       
       {/* Hero Badge - Compacted */}
                  ...
                  return (
                    ...
                    >
                      <div className={`
                        ...
                        ${isSelected ? `border-${agent.color}-500 bg-zinc-900 shadow-[0_0_15px_rgba(var(--color-${agent.color}),0.3)]` : 'border-zinc-700 bg-zinc-950'}
                      `}>
                        {agent.avatar}
                        {isSelected && (
       ...
       {/* --- INPUT FIELD (MOVED UP FOR BETTER VISIBILITY) --- */}
          ...
          <form onSubmit={handleSubmit} className="relative group">
             ...
             <div className="relative flex items-center bg-black border border-zinc-700 p-1 group-focus-within:border-orange-500 transition-colors shadow-2xl">
                ...
                <input
                   ...
                   placeholder={selectedAgents.length === 0 ? "Select at least one councilor..." : "Enter directive..."}
